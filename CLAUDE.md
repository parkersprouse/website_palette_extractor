...
**The name has three forms, and which one to use is decided by role, not by
taste** (renamed from `palettekit` on 2026-08-03, owner decision):

| Form | Used for |
|---|---|
| `Website Palette Extractor` | Prose, headings, the CLI's own description, and the "Generated by" header in every emitted `.css`/`.scss`/`.ts`/`.tailwind.js` |
| `website_palette_extractor` | The **import package** — `import website_palette_extractor`, `python3 -m website_palette_extractor`, every path under this directory, and `website_palette_extractor.pyz`. Underscores because Python identifiers require them |
| `website-palette-extractor` | The **installed console script** and the distribution name in `pyproject.toml`. Hyphens because that is the CLI convention, and PyPI normalises `_` to `-` regardless — the two spellings are the same project to a package index |

Getting these backwards is silent rather than loud: `prog=` and the
`error: … requires Python 3.11+` guard are strings, so a wrong one ships a
command name that does not exist without any test noticing — the tests never
execute either. The cheap check after touching the name anywhere is
...
python3 build.py                         # rebuild website_palette_extractor.pyz (see below)
```
...
rebuilds `website_palette_extractor.pyz`, prints a success line, and never writes `dist/` at
...
Installing exposes a `website-palette-extractor` console script (`[project.scripts]`), so the
three ways to run it are `python3 -m website_palette_extractor`, `website-palette-extractor`, and the zipapp.
...
terminal: stages `website_palette_extractor/` into a temp dir with the `__main__.py` shim
...
(`website_palette_extractor`, `tinycss2`, `cssselect2`, `html5lib`, `webencodings`, `six`)
...
```bash
uv run --python 3.11 --no-project python website_palette_extractor.pyz <target> -o out
```
...
The package keeps its own `__main__.py` so `python3 -m website_palette_extractor` works from a
...
(`website_palette_extractor.PYTHON_FLOOR`, read from one place and checked by
...
--python 3.10 --no-project python website_palette_extractor.pyz …` now prints `error:
website-palette-extractor requires Python 3.11+ (running 3.10.20).` and exits 1, instead of
...
`website_palette_extractor/` is not done until `website_palette_extractor.pyz` has been rebuilt from it.**
...
`website_palette_extractor/`, and **verify with an interpreter that has none of the three
...
way `website_palette_extractor.pyz` does.** `README.md` points readers at it as sample
...
```bash
python3 -m website_palette_extractor parkersprouse.me.har --images -o example
```
...
| `emit.py` | 545 | Emitters; `_HTML` loads the report template off disk (T28) rather than holding it as a string (T20: status sub-headings, T24: always-present Caveats section, T29: `_font_data_uri` inlines the report's two custom fonts as `data:` URIs) |
| `report_template.html` | 1114 | The interactive report's structural HTML/CSS/JS, `__PLACEHOLDER__`-marked (T28) — a real `.html` file, not a Python string, so it gets editor/linter support. Its two `@font-face` blocks (T29) are the only faces the report's own CSS can ever select — see PLAN.md T29 before adding a weight/style back without checking reachability the same way. A 2026-08-04 design pass (T30) restyled headings/controls and turned URL-shaped substrings in the "Extraction report" panel into real links — see T30 for the two bugs that surfaced while documenting it and how they were fixed |
...
`website_palette_extractor.__version__` and moves on its own schedule — an additive key never
...
> `website_palette_extractor.pyz`. It cannot detect a parser regression; the breadth check
...
for u in getbootstrap.com ui.shadcn.com www.djangoproject.com \
  ...
  python3 -m website_palette_extractor "https://$u" -o "out/$u"
done
```
...
were fixes. `git stash push website_palette_extractor/` is enough to run old against new
...
- [x] `pyproject.toml` — hatchling backend, version read from
      `website_palette_extractor/__init__.py`, console script, `images`/`images-fast`/`dev`
      extras, ruff config. Verified by building and installing the wheel.
- [x] `[project.urls]` and `authors` — filled in
      (`github.com/parkersprouse/website_palette_extractor`, Parker Sprouse).
- [x] `.gitignore` — present. ~~**Narrower than earlier revisions of this
      list specified**: it carries `*.har` (with one exception, below),
      `.DS_Store`, `.remember`, `.venv`, `palettes` and `**/*cache*` … `dist/`
      and `out/` are **not** ignored~~ — **replaced 2026-08-03 with GitHub's
      standard Python template**, merged on top of this project's own five
      entries (`*.har`, `!parkersprouse.me.har`, `palettes`, `.remember`,
      `**/*cache*`).

      **What that changed, and the one thing it did not:** `dist/` and
      `build/` are now ignored, so the sentence above is no longer true of
      `dist/` — build artifacts stay out of the tree, which is the normal
      arrangement and worth having. `build/` is directory-only, so **`build.py`
      is unaffected** (checked; it is still tracked). Crucially `*.pyz` is
      still **not** ignored, which is the entry that matters here: it is why
      `website_palette_extractor.pyz` is a tracked, shipped artifact at all,
      and why it can go stale (`PLAN.md` T1). `out/` is likewise still
      tracked-able.
