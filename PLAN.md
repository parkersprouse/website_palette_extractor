...
### T28 — Move the HTML report template out of `emit.py` into its own file — landed 2026-08-04

**Not an accuracy task; a maintainability one, owner-requested.** `emit._HTML`
was a ~600-line triple-quoted string holding the interactive report's HTML,
CSS and JS inline in a `.py` file — no editor or linter treats a Python string
literal as any of those three languages, so it was an opaque blob to every
tool in the project except the one hand-reading it. Moved verbatim to
`website_palette_extractor/report_template.html`; `emit.py` dropped from 1086
lines to 503.

**`emit_html()` itself is unchanged.** It still runs one `re.sub` pass filling
nine `__PLACEHOLDER__` names into `_HTML` (the single-pass design is T27's own
fix, item 1 above, for exactly this templating mechanism) and still returns a
fully self-contained string. Only where `_HTML`'s *value* comes from moved —
from a string literal compiled into the module to a file read at import time.
**Invariant 11 (the emitted report opens from `file://`, no `<link>`, no
`fetch()`) is about the output and is untouched**; it would be a real
regression to read that invariant as now forbidding a template file, so it's
worth stating plainly that this doesn't.

**The one real risk was reading the template file back out of
`website_palette_extractor.pyz`, and it was de-risked before any code moved.**
A path relative to `__file__` — the obvious first draft — works from a
checkout and from an installed wheel and fails only inside the zipapp, where
the package lives inside a zip archive and there is no filesystem path to
`open()`. That is the identical failure shape `build.py`'s own dependency
-vendoring check exists to catch (`PLAN.md` T1, and the `six`-as-top-level
-module story in T27 item 6): invisible on the machine that built it,
guaranteed everywhere else. `importlib.resources.files(__package__)` reads
through the zipimport loader correctly instead — confirmed with a throwaway
probe file vendored into a real rebuilt `.pyz` and read back with
`importlib.resources` on a clean `uv run --python 3.11 --no-project`
interpreter, deleted before the real move, not assumed from the stdlib docs.
`Traversable.read_text()` needs Python 3.11 for the `abc` to carry it
directly — one more place this project's 3.11 floor (`PLAN.md` T2) is load
-bearing rather than nominal, the same shape `tomllib` in `build.py` already
is.

**`build.py`'s `_verify()` gained one more structural assertion**:
`website_palette_extractor/report_template.html` must be a literal member of
the built archive. `shutil.copytree` already carries the file along with
every other non-`.py` file under the package, so this is insurance against a
*future* staging change rather than a fix for an observed gap — but insurance
verified to actually fire: confirmed `_verify()` raises when the file is
temporarily removed from the source tree before a rebuild, and passes again
once restored, the same "prove the check bites" discipline T27 item 6 used
for the `six` fix.

**Packaging was checked, not assumed, for both build targets.**
`[tool.hatch.build.targets.sdist]` already lists `/website_palette_extractor`
as a whole directory, so the new file needed nothing there. The wheel was the
open question — `[tool.hatch.build.targets.wheel] packages = [...]` is a
directory-based strategy, and whether it carries non-`.py` files by default
was unverified until checked directly: built both targets with
`pyproject-build` and confirmed `report_template.html` is present in both the
`.whl` (via `unzip -l`) and the `.tar.gz` (via `tar tzf`).

**Verified at the level this change actually operates on: the HTML output,
not the JSON.** JSON is generated by a completely different code path
(`to_document`) untouched by this move, so a green JSON diff would say
nothing about the one thing that could break — the same "diff at the level
the change operates on" discipline this file states outright under "Breadth
check," applied here to a packaging-shaped change instead of a parsing one.
Ran `parkersprouse.me.har --images` through all three run modes — module,
installed console script, and a clean-3.11-interpreter zipapp with none of
the three core dependencies installed — and diffed the emitted `index.html`
with the `generated` timestamp normalised out (same reason the JSON identity
check strips it): all three byte-identical to each other, and all three
byte-identical to the already-committed `example/index.html`, confirming the
file move changed no emitted byte and `example/` needs no regeneration.
`ruff check .` stayed clean; the `emit.py` `E501` per-file-ignore is still
needed but for a different, smaller reason now (two doc comments whose length
comes from `website_palette_extractor.__version__`'s own name, not the
former 600-line string), and its comment in `pyproject.toml` was corrected
rather than left describing a cause that no longer exists. Full suite (223
→ 224 tests) passes on 3.11, 3.12, 3.13 and 3.14.

Test: `test_template_declares_no_unknown_placeholder` (`tests/test_emit.py`).
The existing `test_every_placeholder_is_still_filled` only checks that the
*known* placeholders got substituted out of the rendered output; it says
nothing about a template edit that introduces a new placeholder-shaped token
with no matching `fills` entry in `emit.py` — `re.sub`'s pattern is built
from `fills`' own keys, so an orphaned `__FOO__` simply never matches and
passes straight through into the shipped report, silently, in whatever site's
report happens to render it first. The new test reads the template's own
placeholder set and asserts it equals the known nine, catching that at the
template-authoring end instead. Required to fail against a template carrying
a stray `__BOGUS_PLACEHOLDER__` before being trusted, per this file's own "a
test that passes before and after tests nothing" discipline.

**A missing `report_template.html` fails at `emit.py` import time, loudly,
and that is left as-is rather than guarded.** `_HTML`'s load happens at
module scope, so a checkout missing the file fails every test in
`tests/test_emit.py` on collection with a traceback naming the missing path,
rather than degrading. No `try`/`except` fallback was added, deliberately: a
missing template is not a recoverable state — there is nothing sensible to
serve instead of the report — and swallowing the failure to produce a softer
message would trade a loud, immediate, correctly-located error for a
